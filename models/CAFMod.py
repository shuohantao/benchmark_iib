...
class CAFMod(nn.Module):
    def __init__(self, shape, depth_1=4, depth_2=4, depth_3=8, modes = 16, prior=Normal(0, 1), dimhid=1024, layer_depth=2, fourier=False, **kwargs):
        ...
        for i in range(depth_1):
            ...
            self.flow.append(CouplingLayerScaleInv(device=torch.device('cuda'), partition_even=[checkerBoard, even], free_net= CNN_Linear(c_in=1, c_out=2, c_hidden=32, shape=torch.Size((1, 1, modes, modes))), c_in=1))
            even = not even
        #PatchFlow(2)
        ...
        for i in range(depth_2):
            ...
            self.flow.append(CouplingLayerScaleInv(device=torch.device('cuda'), partition_even=[checkerBoard, even], free_net=CNN_Linear(c_in=4, c_out=8, c_hidden=64, shape=torch.Size((1, 4, modes//2, modes//2))), c_in=4))
            even = not even
        ...
        for i in range(depth_3):
            ...
            self.flow.append(CouplingLayerScaleInv(device=torch.device('cuda'), partition_even=[checkerBoard, even], free_net=CNN_Linear(c_in=8, c_out=16, c_hidden=128, shape=torch.Size((1, 8, modes//4, modes//4))), c_in=8))
            even = not even
