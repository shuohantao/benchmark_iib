...
class PixelFourier(nn.Module):
    def __init__(
        ...
        self.val_dec = ResNetMLP(2*nde, 64, self.ndv_out, 12)
    ...
    #     scale = torch.maximum(pos2d[:, 0, ...], pos2d[:, 1, ...])/torch.max(pos2d)
    #     scale = torch.exp(scale - 1).unsqueeze(1)
    #     sample_vals, sample_posns, query_truth, query_posns, _, query_scales = self.sample_context(x, pos, scale, num=self.context_num)
    ...
    def forward(self, x):
        ...
        for i in range((res-self.base_res)//self.ar_step):
    ...
    def sample(self, x, num_context, autoregessive=True, num_samples=-1, *args, **kwargs):
    ...
    def sample_normal(self, x, num_context, num_samples, *args, **kwargs):
        ...
        pos[:, 0, :, :] = x_coords / self.base_res
        pos[:, 1, :, :] = y_coords / self.base_res
        sample_truth, sample_posns, query_truth, query_posns, idx, _ = self.sample_context(x, pos, num=num_context)
        x = x.flatten(start_dim=-2, end_dim=-1).permute(2, 0, 1)
        pos = pos.flatten(start_dim=-2, end_dim=-1).permute(2, 0, 1)
        sample_posns = self.pos_enc(sample_posns)
        ...
        query_vals = self.sample_flow(query_vals)
        img = x.clone()
