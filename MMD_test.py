...
train, test = load_mnist(batch_size=cm.config['MMD_test']['batch_size'], targets=list(range(10)), shape_setting=[[max(cm.config["MMD_test"]["resolutions"]), 1]])
...
with open(os.path.join(cm.config["MMD_test"]["pickle_save_dir"], f'{cm.config["MMD_test"]["model_pth"].split("/")[-1].split(".")[0]}_mmds_{total_params}.pkl'), "wb") as f:
    pickle.dump(mmds, f)