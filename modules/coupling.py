...
class CouplingLayerScaleInv(nn.Module):
    def __init__(self, partition_even, device, free_net, c_in):
        super().__init__()
        self.free_net = free_net
        self.scaling_factor = nn.Parameter(torch.zeros(c_in))
        self.epsilon = 1e-5
        ...
        self.device = device
    def forward(self, x, sample=False, condition=None, neurop_condition=None):
        shape = x.shape
        mask = self.partition(shape, self.even).to(self.device)
        x_left = x * mask
        if condition is not None:
            if neurop_condition is not None:
                st = self.free_net(torch.cat([x_left, condition], dim=1), shape=shape, condition=neurop_condition)
            else:
...
class CouplingLayerAR(nn.Module):
    def __init__(self, partition, device, free_net, c_in):
        super().__init__()
        self.free_net = free_net
        self.scaling_factor = nn.Parameter(torch.zeros(c_in))
        self.epsilon = 1e-5
        self.partition = partition
        self.device = device
    def forward(self, x, sample=False, condition=None, inpaint_step=1):
        shape = x.shape
        modes = shape[-1]
        mask = self.partition(modes, inpaint_step).to(self.device)
        x_left = x * mask
        if condition is not None:
            st = self.free_net(torch.cat([x_left, condition], dim=1), shape=shape)
            s, t = st.chunk(2, dim=1)
        ...
        if sample:
            z = (x - t) * torch.exp(-s)
            ldj = None
        z = z + torch.zeros(1, 1, modes+inpaint_step, modes+inpaint_step)
        return z, ldj    
